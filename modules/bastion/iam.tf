...
data "aws_iam_policy_document" "bastion_ec2_role_source" {
  statement {
    sid    = "EC2AssumeRole"
    effect = "Allow"
    principals {
...
resource "aws_iam_policy" "bastion_policy" {
  ...
  path        = "/"
  description = "${var.name_prefix}-Bastion-EC2-Policy"
  policy      = data.aws_iam_policy_document.bastion_policy_document.json
  tags        = { Name = "${var.name_prefix}-Bastion-EC2-Policy" }
}

# EC2 IAM Role
resource "aws_iam_role" "bastion_role" {
  name                = "${var.name_prefix}-Bastion-EC2-Role"
  assume_role_policy  = data.aws_iam_policy_document.bastion_ec2_role_source.json
  managed_policy_arns = ["arn:aws:iam::aws:policy/AdministratorAccess"]
  tags                = { Name = "${var.name_prefix}-Bastion-EC2-Role" }
}
